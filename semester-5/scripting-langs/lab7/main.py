...
most_deaths_country_name = country_names.get_name_by_code(
    most_deaths_country.country_code)
print("Most deaths: " + most_deaths_country_name)

# Most cases
most_cases_country = max(cases_countries)
most_cases_country_name = country_names.get_name_by_code(
    most_cases_country.country_code)
...
print("\nWorst day in the world: %s  %d deaths" %
      (worst_day_date.strftime(DATE_FORMAT), worst_day_deaths))
...
for continent in countries_in_continents.keys():
    ...
    (continent_worst_day_date, continent_worst_day_deaths) = get_worst_day(
        continent_all_days)
