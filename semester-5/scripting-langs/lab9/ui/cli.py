...
def get_place(country_names):
  while True:
    ...
    if closest_place is not None:
      print("Couldn't find the place called %s. Did you mean %s?" % (place, closest_place))
      continue
...
def get_sort():
  choices = ["cases", "deaths", "none"]
  sort_by = get_menu_choice("Sort by", choices, default="none")

  return None if sort_by == "none" else sort_by
