...
PIVOT (
    MIN(imie) FOR lvl IN (2 sz1, 3 sz2, 4 sz3)
);
...
SELECT imie,
       ...
       'powyzej 864' "Dawka"
FROM Kocury
WHERE przydzial_myszy * 12 + myszy_extra * 12 > 864 AND
...
GROUP BY K1.pseudo
HAVING COUNT(DISTINCT K2.przydzial_myszy + NVL(K2.myszy_extra, 0)) < 7
ORDER BY Zjada DESC;
...
-- Zadanie 28
SELECT TO_CHAR(EXTRACT(YEAR FROM w_stadku_od)) Rok,
...
SELECT imie,
       przydzial_myszy + NVL(myszy_extra, 0) "Zjada",
       nr_bandy "Nr bandy",
       srednia "Srednia bandy"
FROM (
    SELECT nr_bandy, AVG(przydzial_myszy + NVL(myszy_extra, 0)) srednia
    FROM Kocury
    GROUP BY nr_bandy
)
JOIN Kocury USING (nr_bandy)
WHERE (przydzial_myszy + NVL(myszy_extra, 0)) <= srednia AND plec = 'M'
...
UPDATE Kocury
SET przydzial_myszy = DECODE(
        plec, 'M', przydzial_myszy + 10,
...
-- Zadanie 33a
SELECT DECODE(plec, 'Kocur', ' ', nazwa) "NAZWA BANDY",
       plec,
       ile,
       szefunio,
       bandzior,
...
(
    ...
    FROM (
        SELECT nazwa, plec, funkcja, przydzial_myszy + NVL(myszy_extra, 0) przydzial
        FROM Kocury NATURAL JOIN Bandy
    ) PIVOT (
