...
namespace WcfServiceHost1
{
    class Program
    {
        ...
        static string baseUri = $"http://localhost:{port}/MyStringOps";
        static void Main(string[] args)
        {
            Uri baseAddress = new Uri(baseUri);
            ServiceHost myHost = new ServiceHost(typeof(MyStringOps), baseAddress);

            WSHttpBinding myBinding = new WSHttpBinding();

            ServiceEndpoint endpoint1 = myHost.AddServiceEndpoint(typeof(IStringOps), myBinding, "endpoint1");
            ...
            try
            {
                ...
                ServiceEndpoint endpoint2 = myHost.AddServiceEndpoint(typeof(IStringOps), binding2, "endpoint2");
